...
Well, $\Pr(A \cap B) = \frac{1}{6}$ since the only outcome that satisfies
...
An obvious approach is to keep calling their API until we find a million
unique CAPTCHAs.
...
Since the number of duplicates $D$ is always non-negative, we satisfy the assumption
of the inequality.
$$
