...
Our goal today is to develop faster Johnson-Lindenstrauss projections. Typically, we use sparse or structured matrices instead of fully random JL matrices because they are faster to compute.
...
<p align="center">
<img src="images/subsampling.png" width="400px">
</p>
...
<p align="center">
<img src="images/uniform_hard.png" width="400px">
</p>

**Claim:**
If $x_i ^2 \leq \frac{c}{n} \| \mathbf{x} \|_2^2$ for all $i$, then
...
We can see that $\mathbf{M}$ must be a random matrix from the observation that $\| \mathbf{Mx} \|_2^2 = \|\mathbf{x}\|_2^2$ (i.e., $\mathbf{M}$ is orthogonal).
...
If $\mathbf{M}$ is known in advance, we can adversarially choose $\mathbf{x}$ to give concentrated mass to the first coordinate.
...
Since we only need to compute $\mathbf{H}_{k-1} \mathbf{x}_a$ and $\mathbf{H}_{k-1} \mathbf{x}_b$ each once, the time to compute $\mathbf{H}_k \mathbf{x}$ is $T(n) = 2T(n/2) + O(n) = O(n \log n)$.

**Property 3**:
The randomized Hadamard matrix $\mathbf{HD}$ is a good mixing matrix for smoothing out vectors.

<p align="center">
<img src="images/hadamard_structured.png" width="300px">
<img src="images/hadamard_randomized.png" width="300px">
</p>
...
::: {.proof-block}
<details open>
<summary>Proof of SHRT Mixing Lemma</summary>
...
\end{align*}
where $D_{1,1}, \ldots, D_{n,n}$ are independent random variables with $D_{j,j} = \pm 1$ chosen uniformly at random.
Equivalently,
\begin{align*}
...
= \frac{1}{n} \sum_{j=1}^n x_j^2 \textrm{Var} \left[ R_j \right] = \frac{1}{n} \sum_{j=1}^n x_j^2 = \frac{1}{n} \| \mathbf{x} \|_2^2.
\end{align*}
...
The algorithm has also been used for accelerating vector dimensionality reduction, linear algebra, locality sensitive hashing, and randomized kernel learning methods.
