...
$$
After a $u$-substitution with $u=1-\sqrt{s}$, we have that the last equality is
$$
\frac{2}{(D+1)(D+2)}.
$$
...
$$
\Pr(|S - \mu| \geq \epsilon \mu) \leq \Pr( |S - \mu| \geq \epsilon \sigma )
...
we take the average $\bar{S} = (S_1 + \ldots + S_k)/k$ and return
the estimate $\hat{D} = \frac{1}{\bar{S}} - 1$.
...
\textrm{Var}\left[
\frac{1}{k}
\sum_{i=1}^k X_i\right]
= \frac{1}{k^2}\sum_{i=1}^k \textrm{Var} \left[X_i\right]
= \frac{\sigma^2}{k}
$$
...
we have that $\mathbb{E}[\bar{S}] = \mu$ as before but now
$\textrm{Var}(\bar{S}) \leq \frac{\mu^2}{k}$.
Then Chebyshev's inequality gives
$$
\Pr\left(|\bar{S} - \mu| \geq c \frac{\mu}{\sqrt{k}} \right) \leq \frac{1}{c^2}.
$$
Setting $c = \frac{1}{\sqrt{\delta}}$ and $k = \frac{1}{\epsilon^2 \delta}$, we have
$$
\Pr(|\bar{S} - \mu| \geq \epsilon \mu) \leq \delta.
$$

We're *nearly* done except that we have only bounded $\bar{S}$
around its expectation.
We really need to bound $\hat{D}$ around its expectation.
We know that with probability $1-\delta$,
$$
(1-\epsilon) \mathbb{E}[\bar{S}]
\leq \bar{S} \leq (1+\epsilon) \mathbb{E}[\bar{S}].
$$
Inverting gives,
$$
\frac{1}{(1+\epsilon) \mathbb{E}[\bar{S}]} \leq \frac{1}{\bar{S}} \leq \frac{1}{(1-\epsilon) \mathbb{E}[\bar{S}]}.
$$
...
$$
(1-2\epsilon) \frac{1}{\mathbb{E}[\bar{S}]} - 1
\leq \frac{1}{\bar{S}} - 1 \leq (1+2\epsilon)
\frac{1}{\mathbb{E}[\bar{S}]} - 1.
$$
Then, for $\mathbb{E}[\bar{S}] \leq \frac12$, we have
$$
(1-4 \epsilon)\left( \frac{1}{\mathbb{E}[\bar{S}]} - 1 \right)
\leq \frac{1}{\bar{S}} - 1
\leq (1+4\epsilon) \left( \frac{1}{\mathbb{E}[\bar{S}]} - 1 \right).
$$
(You can check the inequalities by plotting [here](https://www.desmos.com/calculator/qwxy34rllc).)
Since $D = \frac{1}{\mathbb{E}[\bar{S}]} - 1$ and
$\hat{D} = \frac{1}{\bar{S}} - 1$, we have
