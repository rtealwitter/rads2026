...
\end{align}
for some constant $c$. We'll see a proof of this inequality in the next lecture when we cover the Johnson-Lindenstrauss lemma.

To bound the probability that $|w_1| \leq \epsilon$, we will define two events:
...
\end{align}
Applying the standard Gaussian tail bound to the first probability, we get $\Pr\left( |g_1| > \epsilon \sqrt{d/2} \right) \leq \frac{1}{2^{\left(c \epsilon \sqrt{d/2}\right)^2}}$. The second probability is bounded by $\frac{1}{2^{c d}}$ from the JL inequality above. Substituting these in, we have:
\begin{align}
