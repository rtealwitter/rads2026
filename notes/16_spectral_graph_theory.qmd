...
The main idea of spectral graph theory is to understand graph data by constructing natural matrix representations and studying their spectra.
...
We can formally see this connection through
...
We set $c_i = 1$ if $i \in S$ and $c_i = -1$ otherwise.
Then
\begin{align*}
...
\end{align*}.
The smallest eigenvector is the constant vector $\frac1{\sqrt{n}} \mathbf{1}$.
...
\end{align*}
Since we know that $\mathbf{v}_n = \frac1{\sqrt{n}} \mathbf{1}$,
we know that
\begin{align*}
...
Unfortunately, there are no formal guarantees on the quality of the partitioning and it can fail for worst case input graphs.
...
The approach is analogous to scalar concentration inequalities that we've seen before like Markov's, Chebyshev's, and Chernoff's inequalities.
...
\end{align*}
where $\lambda_1 \geq \ldots \geq \lambda_n$ are the eigenvalues of $\bar{\mathbf{A}}$.
