...
Formally, given a binary vector $\mathbf{x} \in \{0,1\}^d$ representing our song clip in $d$ dimensions, we want to find a nearby fingerprint $\mathbf{y} \in \{0,1\}^d$ in our database.
The first challenge is that our database is possibly huge with $O(nd)$ bits where $n$ is the total number of fingerprints in the library.
...
= \frac{\text{\# non-zero entries in common}}{\text{\# non-zero entries in total}}.
\end{align*}
...
$h_i: \{1, \ldots, d\} \rightarrow [0,1]$ for $i \in [k]$.
For each hash function $h_i$
we'll compute
\begin{align*}
c_i(\mathbf{x}) = \min_{j \in \{1, \ldots, d\}: x_j = 1} h_i(j).
\end{align*}
...
We call the compression $C(\mathbf{x}) = (c_1(\mathbf{x}), \ldots, c_k(\mathbf{x}))$.
...
We have $c_i(\mathbf{x}) = c_i(\mathbf{y})$ if and only if the index hashed to the lowest value is 1 in both $\mathbf{x}$ and $\mathbf{y}$.
Since there are $|\mathbf{x} \cap \mathbf{y}|$ such indices, the probability that $c_i(\mathbf{x}) = c_i(\mathbf{y})$ is the Jaccard similarity.
...
Using Chebyshev's inequality, we can show that $\tilde{J}(\mathbf{x}, \mathbf{y})$ is a good estimate for $J(\mathbf{x}, \mathbf{y})$ with high probability.

**Claim:** As long as $k=O(\frac1{\epsilon^2 \delta})$, then with probability $1-\delta$,
\begin{align*}
...
= J(\mathbf{x}, \mathbf{y}).
\end{align*}
We can reduce the variance of the estimator by increasing the number of hash functions $k$.
The variance of the estimator is
\begin{align*}
\textrm{Var}( \tilde{J}(\mathbf{x}, \mathbf{y}) )
&= \frac1{k^2} \sum_{i=1}^k \textrm{Var}( \mathbb{1}[c_i(\mathbf{x}) = c_i(\mathbf{y})] ) \\
&= \frac1{k^2} \sum_{i=1}^k \left(J(\mathbf{x}, \mathbf{y}) -  J(\mathbf{x}, \mathbf{y})^2\right)
\leq \frac1{4k},
\end{align*}
where we used the fact that the variance of a Bernoulli random variable with parameter $p$ is $p(1-p)$ and that $p(1-p) \leq 1/4$ for all $p \in [0,1]$.

How big should we choose $k$ so that the estimate is with an additive error $\epsilon$ of its expectation with probability $1-\delta$?
...
To convince yourself it is possible to find nearest neighbors in sublinear time, consider the problem in one dimension.
...
&= J(\mathbf{x}, \mathbf{y}) + (1-J(\mathbf{x}, \mathbf{y})) \frac1{m} \\
&= J(\mathbf{x}, \mathbf{y}) \frac{m-1}{m} + \frac1{m}\\
&\approx J(\mathbf{x}, \mathbf{y}),
\end{align*}
...
* **False negative rate**: What's the probability we do not find a vector that is close to $\mathbf{x}$?  
* **False positive rate**: What's the probability that a vector in the slot is not close to $\mathbf{x}$?
...
\cdots
\Pr(h_t(\mathbf{x}) \neq h_t(\mathbf{y}))
\approx 1-\left(1-J(\mathbf{x}, \mathbf{y})\right)^t.
\end{align*}
...
Then define the LSH hash function as $h(\mathbf{x}) = g(c_1(\mathbf{x}), \ldots, c_r(\mathbf{x}))$.
We refer to $r$ as the number of *rows*.
...
\end{align*}
The probability that we find $\mathbf{y}$ is
\begin{align*}
1 - \Pr(h_1(\mathbf{x}) \neq h_1(\mathbf{y}))
\cdots
\Pr(h_t(\mathbf{x}) \neq h_t(\mathbf{y}))
\approx 1-\left(1-J(\mathbf{x}, \mathbf{y})^r\right)^t.
\end{align*}

The full LSH scheme now has two parameters: $r$ the number of rows and $t$ the number of tables.
...
If we increase the number of rows $r$, we increase the false negative rate and decrease the false positive rate.
...
Let's design a LSH scheme for cosine similarity with $r$ rows.
...
Let $f: \{-1, 1\}^r \to \{1, \ldots, m\}$ be a uniform random hash function, and $\textrm{sign}(y) = 1$ if $y \geq 0$ and $-1$ otherwise.
...
\begin{align*}
\Pr( h(\mathbf{x}) = h(\mathbf{y}) )
= \left(1 - \frac{\theta}{\pi}\right)^r 
+ \left(1-\left(1 - \frac{\theta}{\pi}\right)^r\right) \frac{1}{m}.
\end{align*}

As an intermediate result, we will show that
\begin{align*}
\Pr_{\mathbf{g} \sim \mathcal{N}(0,1)^d}\left( \text{sign}(\langle \mathbf{g}, \mathbf{x} \rangle) = \text{sign}(\langle \mathbf{g}, \mathbf{y} \rangle) \right)
= 1 - \frac{\theta}{\pi}.
\end{align*}

We will first show the result in two dimensions.
...
For a database of binary vectors in Hamming space, if there exists some vector $\mathbf{y}$ in the database with $\|\mathbf{x} - \mathbf{y}\|_0 \leq R$, then we can return a vector $\hat{\mathbf{y}}$ with $\|\mathbf{x} - \hat{\mathbf{y}}\|_0 \leq C \cdot R$ in $O(d \cdot n^{1/C})$ time and $O(d \cdot n^{1+1/C})$ space.
