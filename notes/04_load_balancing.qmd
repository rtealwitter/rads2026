...
access to a uniformly random hash function $h$,
meaning $h$ is drawn uniformly at random from all possible functions
$\mathcal{U} \rightarrow \{1, \ldots, n\}$.
In particular, for any distinct $x \neq y$, $h(x)$ and $h(y)$ are independent
uniform random variables on $\{1, \ldots, n\}$, which implies
...
$$
Since $h(x_j)$ and $h(x_{j'})$ are independent for distinct inputs $x_j \neq x_{j'}$,
the indicators $s_{i,j}$ and $s_{i,j'}$ are independent for $j \neq j'$.
We can therefore apply linearity of variance and get
$$
\textrm{Var}(s_i)
